# v2.2.6.1 (2026年09月25日)

## 打包修正：一键启动器进源码包

- `Run.exe`（10 KB 的 .NET 启动器，负责装依赖并打开 Web 配置页）此前只在 GitHub 仓库里，PyPI 的两份制品都没有它；现在通过 `MANIFEST.in` 把它和仓库根目录的 `__init__.py` 一起纳入 **sdist**。
- `__init__.py` 里的 `os.system('Run.exe')` 改为按**自身所在目录**用绝对路径调用：原来的写法要求调用者的当前目录正好是项目目录，从别处 `import` 会静默失败。非 Windows 或文件缺失时安静跳过，不影响正常导入。
- 说明：wheel 不适合放启动器 —— pip 装出来的目录里没有 `requirements.txt` 与项目源码，启动器第一步就会失败；需要用启动器的用户请下载源码包（tar.gz）解压后运行。
